...
std::vector<AST::Nitem*> get_args(tokenize&tok){
  ...
  if(tok.top().symbol!=symbol_t::LPAREN) tok.error_throw("関数呼び出しの引数が(で囲まれていません");
  tok.next_token(); // (を消費
  ...
  while(true){
    ...
    else tok.error_throw("関数呼び出しの引数が,で区切られていません");
  }
  if(tok.top().symbol!=symbol_t::RPAREN) tok.error_throw("関数呼び出しの引数が)で囲まれていません");
  tok.next_token();
  return args;
}
...
std::pair<AST::Nitem*,AST::Nitem*> get_right_args(tokenize&tok){ // {上付き，下付き}
  ...
  auto get_arg=[&tok](AST::Nitem*&arg,bool is_below)->void {
    ...
    if(tok.top().symbol==symbol_t::LCURLY){
      ...
      if(tok.top().symbol!=symbol_t::RCURLY) tok.error_throw("{}が閉じられていません");
      tok.next_token();
    }else{
      if(is_below&&is_below_declare) tok.error_throw("下付きの引数が宣言されていません");
      arg=atom(tok);
    }
  };
...
AST::Nitem* atom(tokenize&tok) {
  if(token_t::EMPTY==tok.top().type)
    ...
    tok.next_token();
    AST::Nitem* ret=expr(tok);
    if(tok.top().symbol!=symbol_t::RPAREN) tok.error_throw("閉じかっこ ')' がありません");
    tok.next_token();
    return ret;
