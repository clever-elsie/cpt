...
## 言語仕様
### データ型
- 多倍長整数, 多倍長浮動小数点, 真偽値, 複素数, 行列（行・列ベクトルを含む）, 範囲(Range), 関数オブジェクト(ラムダ), 文字列
- 複素数は `1 + 2i` のようにサフィックス `i` を用いて記述
- ベクトル・行列は `[1, 2; 3, 4]` のような MATLAB 風表記をサポート

### 変数宣言・代入
- `let x=expr1, y=expr2;` または `let x:=expr;` で宣言可能（カンマ区切りで複数宣言可、値は `void`）
- 代入は `:=`（`=`は宣言時のみ）
- スコープは関数・ブロック単位で管理
...
- 線形代数: `\T(A)` (転置), `\t(A)` (共役転置), `\dot(A, B)` (要素積), `\vector(n)` (列ベクトル初期化), `\rowvector(n)` (行ベクトル初期化)
